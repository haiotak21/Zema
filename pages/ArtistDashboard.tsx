...
import { Upload, FileText, Music, CheckCircle, Clock, Image as ImageIcon, X, User, BadgeCheck, PlayCircle, PauseCircle, Trash2, Plus, Save, ArrowLeft, RefreshCw, SkipBack, SkipForward, Mic2, AlertCircle, BarChart3, Globe, DollarSign, Calendar, MapPin, Wand2, Youtube, Instagram, Twitter, ExternalLink, HelpCircle, MessageSquare, ShoppingBag, CreditCard, Users, Share2, Megaphone, Video, Send, Download, Copy, Percent, Wallet, MousePointer2, Bell, Scale, FileCheck, ShieldAlert, Gavel, Check, UserPlus, Briefcase, Lock, Shield, FileQuestion, AlertTriangle, ChevronRight, Disc, Info, ListMusic } from 'lucide-react';
...
const MOCK_ANALYTICS = {
    ...
    payoutHistory: [
        { id: 'tx-101', date: 'Oct 25, 2023', amount: '12,450 ETB', method: 'CBE', status: 'Processing' },
        { id: 'tx-099', date: 'Sep 28, 2023', amount: '10,200 ETB', method: 'Dashen', status: 'Paid' },
        { id: 'tx-082', date: 'Aug 30, 2023', amount: '8,500 ETB', method: 'Telebirr', status: 'Paid' },
    ]
};

export const ArtistDashboard: React.FC<ArtistDashboardProps> = ({ user }) => {
  const [activeTab, setActiveTab] = useState<'stats' | 'upload' | 'lyrics' | 'community' | 'money' | 'collab' | 'marketing' | 'rights' | 'team' | 'support' | 'alerts' | 'profile'>('stats');
  const [isUploading, setIsUploading] = useState(false);

  // Local state for recent uploads simulation
  const [recentUploads, setRecentUploads] = useState([
     { id: 'u1', title: "Gue", status: "Live", date: "2 days ago", type: 'Single', streams: '124K' },
     { id: 'u2', title: "Addis Night", status: "Pending", date: "5 hours ago", type: 'Single', streams: '-' },
     { id: 'u3', title: "Untitled Demo", status: "Rejected", date: "1 week ago", type: 'EP', streams: '-' },
  ]);
  ...
  const [files, setFiles] = useState<{ audio: File | null, cover: File | null }>({ audio: null, cover: null });
  const [previews, setPreviews] = useState<{ audio: string | null, cover: string | null }>({ audio: null, cover: null });
  ...
  const [lyricLines, setLyricLines] = useState<{id: string, time: number, text: string}[]>([]);
  ...
  // --- UPLOAD HANDLERS ---
  const handleFileChange = (type: 'audio' | 'cover', e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handlePublish = () => {
      ...
      setTimeout(() => {
          ...
          setUploadStep(1);
          setFormData({ title: '', genre: 'Traditional', description: '', isExplicit: false, releaseDate: '' });
          setFiles({ audio: null, cover: null });
          setPreviews({ audio: null, cover: null });
          setActiveTab('stats');
          alert("Release submitted successfully! It will be live after moderation.");
      }, 2000);
  };
      ...
      reader.onload = (event) => {
          ...
          const lines = content.split('\n').map((line, idx) => {
              ...
              if (match) {
                  ...
                  return { id: `l-${idx}`, time: min * 60 + sec, text: match[3].trim() };
              }
  ...
  return (
    <div className="flex h-screen bg-zinc-50 dark:bg-black overflow-hidden">
                ...
                {(user as any)?.pendingArtist || (user as any)?.requestedRole === 'ARTIST' ? (
                    <div className="fixed top-16 left-72 right-8 z-50">
                        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-lg shadow">
                            ...
                            Your artist application is under review. You currently have a listener account — you'll be upgraded once approved.
                        </div>
                    </div>
                ) : null}
        {/* Sidebar */}
        ...
        <div className="flex-1 overflow-y-auto bg-zinc-50 dark:bg-black p-8">
            ...
            {activeTab === 'stats' && (
                <div className="space-y-8 animate-in fade-in">
                    <div>
                        <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-1">Good morning, {user?.name?.split(' ')[0]}</h2>
                        <p className="text-zinc-500">Here's what's happening with your music today.</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            { label: 'Total Streams', value: '1.2M', change: '+12%', icon: PlayCircle, color: 'text-blue-500' },
                            { label: 'Listeners', value: '45.2K', change: '+5%', icon: Users, color: 'text-purple-500' },
                            { label: 'Followers', value: '12.8K', change: '+2%', icon: UserPlus, color: 'text-rose-500' },
                            { label: 'Revenue', value: '450K ETB', change: '+8%', icon: Wallet, color: 'text-emerald-500' },
                        ].map((stat, i) => (
                            <div key={i} className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800">
                                <div className="flex justify-between items-start mb-4">
                                    <div className={`p-2 rounded-lg bg-zinc-100 dark:bg-zinc-800 ${stat.color}`}>
                                        <stat.icon size={20} />
                                    </div>
                                    <span className="text-emerald-500 text-xs font-bold bg-emerald-50 dark:bg-emerald-900/20 px-2 py-1 rounded">{stat.change}</span>
                                </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800">
                            <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-6">Recent Uploads</h3>
                            <div className="space-y-4">
                                {recentUploads.map((upload, i) => (
                                    <div key={i} className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950/50 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-950 transition-colors">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 bg-zinc-200 dark:bg-zinc-800 rounded flex items-center justify-center text-zinc-400">
                                                <Music size={20} />
                                            </div>
                                            <div>
                                                <p className="font-bold text-sm text-zinc-900 dark:text-white">{upload.title}</p>
                                                <p className="text-xs text-zinc-500">{upload.type} • {upload.date}</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <span className={`inline-block px-2 py-1 rounded text-xs font-bold ${
                                                upload.status === 'Live' ? 'bg-emerald-100 text-emerald-600' :
                                                upload.status === 'Pending' ? 'bg-yellow-100 text-yellow-600' :
                                                'bg-red-100 text-red-600'
                                            }`}>
                                                {upload.status}
                                            </span>
                                            {upload.status === 'Live' && <p className="text-xs text-zinc-500 mt-1">{upload.streams} streams</p>}
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800">
                            <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-6">Audience Location</h3>
                            <div className="space-y-4">
                                {MOCK_ANALYTICS.locations.map((loc, i) => (
                                    <div key={i}>
                                        <div className="flex justify-between text-sm mb-1">
                                            <span className="text-zinc-700 dark:text-zinc-300">{loc.city}</span>
                                            <span className="font-bold text-zinc-900 dark:text-white">{loc.percent}%</span>
                                        </div>
            ...
            {activeTab === 'upload' && (
                <div className="max-w-3xl mx-auto animate-in fade-in">
                    <div className="mb-8">
                        <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">Upload New Music</h2>
                        <div className="flex items-center gap-2 text-sm text-zinc-500 mt-2">
                            <span className={uploadStep >= 1 ? "text-rose-600 font-bold" : ""}>1. Details</span>
                            <ChevronRight size={14} />
                            <span className={uploadStep >= 2 ? "text-rose-600 font-bold" : ""}>2. Assets</span>
                            <ChevronRight size={14} />
                            <span className={uploadStep >= 3 ? "text-rose-600 font-bold" : ""}>3. Review</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-8 shadow-sm">
                        {uploadStep === 1 && (
                            <div className="space-y-6">
                                <div>
                                    <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Release Type</label>
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Primary Genre</label>
                                        <select 
                                ...
                                <div className="flex justify-end">
                                    <Button onClick={() => setUploadStep(2)} disabled={!formData.title}>Next Step</Button>
                                </div>
                            </div>
                        )}
                        ...
                        {uploadStep === 2 && (
                            <div className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Cover Art</label>
                                        <div className="aspect-square bg-zinc-100 dark:bg-zinc-950 border-2 border-dashed border-zinc-300 dark:border-zinc-800 rounded-xl flex flex-col items-center justify-center relative overflow-hidden group">
                                            {previews.cover ? (
                                                <>
                                                    ...
                                                    <button 
                                                        onClick={(e) => { e.preventDefault(); handleRemoveFile('cover'); }}
                                                ...
                                                <div className="text-center p-4">
                                                    ...
                                                    <p className="text-xs text-zinc-500">Click to upload (3000x3000px)</p>
                                                </div>
                                            )}
                                            {!previews.cover && (
                                                <input 
                                        ...
                                        <div className="h-full bg-zinc-100 dark:bg-zinc-950 border-2 border-dashed border-zinc-300 dark:border-zinc-800 rounded-xl flex flex-col items-center justify-center relative p-6 group">
                                            {files.audio ? (
                                                <div className="text-center w-full">
                                                    ...
                                                    <p className="text-sm font-bold text-zinc-900 dark:text-white truncate max-w-[200px] mx-auto">{files.audio.name}</p>
                                                    <p className="text-xs text-zinc-500">{(files.audio.size / 1024 / 1024).toFixed(2)} MB</p>
                                                    <button 
                                                        onClick={(e) => { e.preventDefault(); handleRemoveFile('audio'); }}
                                                        className="absolute top-2 right-2 p-1 bg-red-500 rounded-full text-white shadow-lg hover:bg-red-600 transition-colors z-20"
                                                        title="Remove audio"
                                                    >
                                                        <X size={16} />
                                                ...
                                                <div className="text-center">
                                                    ...
                                                    <p className="text-xs text-zinc-500">WAV or FLAC preferred</p>
                                                </div>
                                            )}
                                            {!files.audio && (
                                                <input 
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <Button onClick={() => setUploadStep(3)} disabled={!files.audio || !files.cover}>Next Step</Button>
                                </div>
                            </div>
                        )}
                        ...
                        {uploadStep === 3 && (
                            <div className="space-y-6 text-center">
                                ...
                                <h3 className="text-xl font-bold text-zinc-900 dark:text-white">Ready to Publish?</h3>
                                <p className="text-zinc-500 max-w-md mx-auto">
                                    Your release "{formData.title}" will be submitted for review. This usually takes 24-48 hours.
                                </p>
                                ...
                                <div className="bg-zinc-50 dark:bg-zinc-950 rounded-lg p-4 text-left max-w-sm mx-auto border border-zinc-100 dark:border-zinc-800">
                                    <div className="flex justify-between text-sm mb-2">
                                        ...
                                        <span className="font-medium text-zinc-900 dark:text-white">{formData.title}</span>
                                    </div>
                                    <div className="flex justify-between text-sm mb-2">
                                        <span className="text-zinc-500">Type:</span>
                                        <span className="font-medium text-zinc-900 dark:text-white">{releaseType}</span>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                        <span className="text-zinc-500">Genre:</span>
                                        <span className="font-medium text-zinc-900 dark:text-white">{formData.genre}</span>
                                    </div>
                                </div>

                                <div className="flex justify-center gap-4 pt-4">
                                    <Button variant="secondary" onClick={() => setUploadStep(2)} disabled={isUploading}>Back</Button>
            ...
            {activeTab === 'lyrics' && (
                <div className="space-y-6 animate-in fade-in relative">
                    <div className="flex justify-between items-center">
                        <div>
                            <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">Lyrics Editor</h2>
                            <p className="text-zinc-500 text-sm">Sync lyrics for your tracks.</p>
                        </div>
                        <div className="flex gap-2">
                            <Button variant="secondary" size="sm" onClick={() => fileInputRef.current?.click()}>
                                <Upload size={14} className="mr-2" /> Import LRC
                            </Button>
                            <input type="file" ref={fileInputRef} className="hidden" accept=".lrc,.txt" onChange={handleImportLyrics} />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[600px]">
                        ...
                        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl flex flex-col overflow-hidden">
                            <div className="p-3 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex items-center justify-between">
                                <span className="text-xs font-bold uppercase text-zinc-500">Time-Synced Lyrics</span>
                                <span className="text-xs text-zinc-400">{lyricLines.length} lines</span>
                            </div>
                            <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-zinc-50 dark:bg-black" ref={editorScrollRef}>
                                {lyricLines.length > 0 ? (
                                    lyricLines.map((line, idx) => (
                                        <div key={line.id} className="flex gap-2 items-center group">
                                            <input 
                                                type="text" 
                                            ...
                                            <button 
                                                ...
                                                onClick={() => setLyricLines(lyricLines.filter(l => l.id !== line.id))}
                                            >
                                                <X size={14} />
                                            </button>
                                        </div>
                            ...
                            <div className="p-3 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex justify-center">
                                <button 
                                    ...
                                    onClick={() => setLyricLines([...lyricLines, { id: Date.now().toString(), time: 0, text: "New line" }])}
                                >
                                ...
                                {editingSong ? (
                                    <div className="space-y-4 w-full">
                                        <div className="flex items-center gap-4 bg-black/30 p-4 rounded-xl text-left border border-white/10">
                                            ...
                                            <div>
                                                <h4 className="font-bold text-white">{editingSong.title}</h4>
                                                <p className="text-sm text-zinc-400">{editingSong.type || 'Single'}</p>
                                            </div>
                                            <button 
                                    ...
                                    <>
                                        ...
                                        <div>
                                            <h3 className="text-white font-bold text-xl">Select a track to sync</h3>
                                            <p className="text-zinc-400 text-sm mt-1">Preview mode</p>
                                        </div>
                                        <div className="flex justify-center gap-4">
                                            <Button onClick={() => setShowSongSelector(true)}>Select Track</Button>
                                        </div>
                                    </>
                                )}
                            </div>
                    ...
                    {showSongSelector && (
                        <div className="absolute inset-0 bg-white dark:bg-zinc-950 z-20 rounded-xl p-6 border border-zinc-200 dark:border-zinc-800 flex flex-col animate-in fade-in zoom-in-95">
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="font-bold text-xl text-zinc-900 dark:text-white">Select a Track</h3>
                                <button onClick={() => setShowSongSelector(false)} className="p-2 bg-zinc-100 dark:bg-zinc-800 rounded-full text-zinc-500">
                                    <X size={20} />
                                </button>
                            </div>
                            <div className="flex-1 overflow-y-auto space-y-2">
                                {recentUploads.length > 0 ? recentUploads.map(song => (
                                    <div 
                                        key={song.id} 
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">
                                            <h4 className="font-bold text-zinc-900 dark:text-white">{song.title}</h4>
                                            <p className="text-xs text-zinc-500">{song.type} • {song.date}</p>
                                        </div>
            ...
            {activeTab === 'profile' && (
                <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in">
                    <div className="flex items-center justify-between">
                        <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">Artist Profile</h2>
                    ...
                    <div className="space-y-6">
                        <div>
                            <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Stage Name</label>
                            <input 
                                type="text"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">Social Links</label>
            ...
            {activeTab === 'money' && (
                <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in">
                    <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">Earnings & Payouts</h2>
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
                        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800">
                            <h4 className="font-bold text-zinc-900 dark:text-white">Payout History</h4>
                        </div>
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                                {MOCK_ANALYTICS.payoutHistory.map((tx) => (
                                    <tr key={tx.id}>
                                        <td className="p-4 text-zinc-900 dark:text-white">{tx.date}</td>
                                        <td className="p-4 font-mono text-zinc-600 dark:text-zinc-300">{tx.amount}</td>
                                        <td className="p-4 text-zinc-500">{tx.method}</td>
                                        <td className="p-4 text-right">
                                            <span className={`px-2 py-1 rounded text-xs font-bold ${
                                                tx.status === 'Paid' ? 'bg-emerald-100 text-emerald-600' : 'bg-yellow-100 text-yellow-600'
                                            }`}>
                                                {tx.status}
                                            </span>
                                        </td>
